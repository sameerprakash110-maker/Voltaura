...
**This dataset is synthetic.** It stands in for the IoT telemetry that a real VOLTAURA
...
| Rows | 15,120 energy + 15,120 water = **30,240 intervals** |
...
|---|---|---|---|---|---|---|
| `ADMIN` | Admin Block | Administrative | 4,200 | 4 | 180 | 09:00–18:00 |
| `ESB` | ESB Block | Academic / Laboratory | 7,800 | 5 | 420 | 08:00–18:00 |
| `APEX` | Apex Block | Academic / Data Centre | 6,100 | 4 | 380 | 08:00–19:00 |
| `LHC` | Lecture Hall Complex | Academic / Study | 5,200 | 3 | 260 | 08:00–22:00 |
| `MPB` | Multipurpose Block | Amenity | 3,800 | 2 | 300 | 08:00–21:00 |
...
|---|---|---|---|---|
| `ESB` | `HVAC_OVERRUN` | Energy | −26 → live | AHU runtime forced to 78–96% duty at 18:00–23:59 when occupancy < 25% |
| `LHC` | `WATER_LEAK` | Water | −22 → live | Constant +41 L/h on the distribution branch, 24/7 |
| `ADMIN` | `LIGHTING_AFTER_HOURS` | Energy | −18 → live | Lighting runtime forced to 74–95% at 19:00–02:59 |
| `APEX` | `EQUIPMENT_FAULT` | Energy | −74 → −52 | Constant +17 kW parasitic draw, 24/7 |
| `MPB` | `PUMP_OVERRUN` | Water | −62 → −41 | Pump forced to 82–100% duty at 22:00–05:59, **plus** roof-tank overflow at ~130 L/h |
...
|---|---|---|---|---|---|
| `ESB` | 18:00–23:59 | 62 → 102 kWh (**+64%**) | 7.2% | 15.8 → **47.3** min/h | 15.8 → 15.8 min/h |
| `ADMIN` | 19:00–02:59 | 20.4 → 32.4 kWh (**+59%**) | 2.4% | 7.4 → 7.4 min/h | 10.6 → **50.6** min/h |
| `LHC` | 01:00–04:59 | 8.8 → 49.8 L/h (**+467%**) | 0.5% | — | — |
...
| Server-room base load (`APEX`) | 26 kW always-on | consistent with a campus data room |
...
| Path | Contents |
|---|---|
| `voltaura.db` | SQLite database — all tables (created by `scripts/seed.py`) |
